...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    sublists = [list(specials)] + [sentence.split(" ") for sentence in sentences]
    unique_words = list(dict.fromkeys(word for sublist in sublists for word in sublist))
    t_to_i = {token:i for i, token in enumerate(unique_words)}
...
def build_id_to_token_vocab(token_to_id):
    ...
    return {val:key for key, val in token_to_id.items()}
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for word in words:
        ...
        else: encoded.append(token_to_id[unk_token])
    return encoded
...
def pad_id_sequence(ids, max_len, pad_id):
    if ids == []: return [pad_id for i in range(max_len)]
    if len(ids) > max_len:
        return ids[:max_len]
    else:
        for i in range(max_len-len(ids)): ids.append(pad_id)
    return ids
